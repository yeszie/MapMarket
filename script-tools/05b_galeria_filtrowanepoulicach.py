...
# Funkcja do wyodrębniania nazwy ulicy
def extract_street(address):
    # Pobieranie pierwszego członu przed przecinkiem
    first_part = address.split(',')[0]
    # Usuwanie numerów z nazwy ulicy
    street_name = re.sub(r'\d+', '', first_part).strip()
    return street_name

# Tworzenie unikalnych adresów na podstawie nazwy ulicy
unique_addresses = {}
for item in data:
    ...
    if street:
        ...
        if street not in unique_addresses:
            unique_addresses[street] = item
        else:
            # Porównanie daty modyfikacji
            existing_item = unique_addresses[street]
...
<head>
    ...
    <meta name="description" content="Galeria zdjęć z lokalizacji, gdzie każde zdjęcie jest unikalne według ulic, wyświetlane w porządku najnowszych dat modyfikacji">
    <meta name="keywords" content="zdjęcia, galeria, lokalizacja, zdjęcia lokalizacji">
    <title>MapMarket.pl - Galeria zdjęć według ulic</title>
    <style>
        .gallery {
            display: flex;
            flex-wrap: wrap;
...
<body>
    <h1>Galeria zdjęć z lokalizacji, gdzie każde zdjęcie jest unikalne według ulic, wyświetlane w porządku najnowszych dat modyfikacji</h1> 
...
for item in unique_addresses.values():
    ...
    # Link do mapy
    map_link = f"/#18/{latitude}/{longitude}"
...
html_file_path = "galeria_zdjec_wedlug_ulic.html"
